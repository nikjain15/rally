...
| 2 | How the AI works (context, grounding) | 4/5 | Assistant grounds in what the caller could already read (channels, commitments, memory) via SAFE tools; system prompt injects private + shared memory (`lib/assistant-run.ts`). Detection prompt is tightly scoped and its output schema-validated (`extractJson`). | Temperature/token budgets are defaults, not tuned; no explicit retrieval ranking for channel summaries (loads then summarizes). |
...
| 5 | Orchestration & routing (multi-model, cost) | 3/5 | Per-task model routing: `opus-4-8` for the Brief, `sonnet-5` elsewhere (`lib/agent.ts`). Single wrapper for all calls keeps cost/observability in one place. | Routing is static (hardcoded per feature), not dynamic on difficulty/cost; no token accounting or budget caps in code. |
| 6 | RAG & context (retrieval, failure modes) | 3/5 | Context is assembled from Firestore (membership-scoped reads, personal unread bookmarks, private + shared memory merged in `runAssistant`). Failure modes are first-class: missing key -> null, bus hiccup -> best-effort, model failure -> deterministic fallback. | No vector store / embeddings; channel summarization loads recent messages rather than retrieving semantically. Fine at cohort scale, not at workspace scale. |
...
- **Model tiers** (`MODELS`): `claude-opus-4-8` for the Brief (the highest-judgment classify
  task), `claude-sonnet-5` for detection and the assistant. Static per-feature routing.
