# Rally, Technical Notes and Self-Assessment

Engineering detail behind Rally, plus a 12-point self-assessment with file-level evidence and gaps.

**Read the scores as the author's own, not a result.** Every number below was awarded by Rally's
author to Rally, with no external reviewer, no rubric supplied by anyone else, and no calibration
against other submissions. A self-awarded 5/5 is a claim, not a measurement. What is checkable is
the evidence column (file paths you can open) and the gap column (deliberately blunt, and the more
useful half of the table). If you only read one column, read the gaps.

## 12-point self-assessment

| # | Dimension | Self-score | Evidence (file refs) | Gap |
|---|---|---|---|---|
...
| 7 | Evals & grounding | 3/5 | ~200 tests across unit/evals/rules/integration/e2e; the rules suite *is* an anti-gaming eval; adversarial "break it" integration pass; documented perf pass. Untrusted model output schema-validated before trust. **A precision/recall/F1 detection eval is now implemented** (`npm run test:evals`) against a committed 50-case labeled set, measuring the deterministic baseline at precision 0.83 / recall 0.92 / F1 0.87 / FP-rate 0.14. A regression guard also pins "model never worse than baseline". | The dataset is 50 hand-written synthetic cases, not sampled from production. The "never worse" guard passes by construction in CI, where no model key is present and the model layer falls back to the baseline, so the model-vs-baseline delta is still unmeasured; publishing one run with a live key is the next step. LLM-judge (brief/assistant faithfulness) and A/B remain roadmap (designed in `EVALS.md`). |
...
The self-scores sum to 47 of a possible 60, but that total is not evidence of anything: it is the
author grading their own work, so treat it as a summary of where attention went rather than a
measured standing. The evidence-backed reading of the table is that the work concentrated on
guardrails, code quality, and the product layer, and the open gaps are the model-vs-baseline eval
(unmeasured until an eval run with a live key is published), dynamic and budget-aware routing, and
anything beyond single-cohort scale.
